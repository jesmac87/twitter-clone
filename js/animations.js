$(document).ready(function(){
	...
	$('#tweet-submit').on('click', function(){
		...
		$('#stream').prepend('<div class = "tweet"> ' +
								'<div class="content">' +
									'<img class="avatar" src="img/Jessem Face.png">' +
									'<strong class="fullname">Jesse Mace</strong>' +
									'<span class="username"> @JMDevMnt</span>' +
									'<p class="tweet-text"></p>' +
								'</div>' +
							'</div>' );
		//add the reply, tweet, favorite, more actions to tweet.
		$('#stream	div:first').find('.content').append('<div class="tweet-actions">'+
															'<ul>'+
															...
															'</ul>'+
														'</div>')
		//add the stats to the tweet
		$('#stream	div:first').find('.content').append('<div class="stats">'+
															'<div class="retweets"><p class="num-retweets">0</p><p>RETWEETS</p></div>'+
															'<div class="favorites"><p class="num-favorites">0</p><p>FAVORITES</p></div>'+
															'<div class="users-interact">'+
																'<div><img class="thumbnail" data-toggle="tooltip" title="Jesse Mace" src="img/JesseM Face.png">'+
																'<img src="img/vklimenko.jpg"></div>'+
															'</div>'+
															'<div class="timeago" title=""></div>'+
														'</div>');
		//add the message to the newly created tweet
		$('#stream div:first').find('.tweet-text').prepend(message);
		//timeago
		var date = new Date().toISOString();
		$('.timeago').attr('title' , date);
		$('.timeago').timeago();
	...
	$(document.body).on('click', '.content', function(){
		$(this).find('.stats').animate({height: "toggle"}, 800);
	});

	//Bootstrap tooltip integration
	$(document.body).on('mouseover', '.thumbnail', function(){
		$(this).tooltip();
	});
